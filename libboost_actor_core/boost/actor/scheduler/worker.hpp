...
#ifndef BOOST_ACTOR_SCHEDULER_WORKER_HPP
#define BOOST_ACTOR_SCHEDULER_WORKER_HPP
...
#include "boost/actor/detail/double_ended_queue.hpp"
...
public:
  ...
  void start() {
    BOOST_ACTOR_ASSERT(this_thread_.get_id() == std::thread::id{});
    auto this_worker = this;
    this_thread_ = std::thread{[this_worker] {
      BOOST_ACTOR_LOGF_TRACE("id = " << this_worker->id());
      this_worker->run();
    }};
  }
  ...
  void external_enqueue(job_ptr job) {
    BOOST_ACTOR_ASSERT(job != nullptr);
    BOOST_ACTOR_LOG_TRACE("id = " << id() << " actor id " << id_of(job));
...
private:
  void run() {
    BOOST_ACTOR_LOG_TRACE("worker with ID " << id_);
    // scheduling loop
    for (;;) {
      auto job = policy_.dequeue(this);
      BOOST_ACTOR_ASSERT(job != nullptr);
      BOOST_ACTOR_LOG_DEBUG("resume actor " << id_of(job));
...
#endif // BOOST_ACTOR_SCHEDULER_WORKER_HPP
