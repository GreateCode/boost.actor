...
#ifndef BOOST_TEST_MESSAGE_HPP
#define BOOST_TEST_MESSAGE_HPP
...
public:
  ...
  const T& get_as(size_t p) const {
    BOOST_ACTOR_ASSERT(match_element(p, detail::type_nr<T>::value, &typeid(T)));
