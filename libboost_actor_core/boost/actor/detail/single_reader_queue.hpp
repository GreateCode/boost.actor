...
#ifndef BOOST_ACTOR_DETAIL_SINGLE_READER_QUEUE_HPP
#define BOOST_ACTOR_DETAIL_SINGLE_READER_QUEUE_HPP
...
#include "boost/actor/detail/intrusive_partitioned_list.hpp"
...
public:
  ...
  enqueue_result enqueue(pointer new_element) {
    BOOST_ACTOR_ASSERT(new_element != nullptr);
...
private:
  ...
  bool fetch_new_data(pointer end_ptr) {
    BOOST_ACTOR_ASSERT(end_ptr == nullptr || end_ptr == stack_empty_dummy());
    pointer e = stack_.load();
    // must not be called on a closed queue
    BOOST_ACTOR_ASSERT(e != nullptr);
    // fetching data while blocked is an error
    BOOST_ACTOR_ASSERT(e != reader_blocked_dummy());
    ...
    while (e != end_ptr) {
      if (stack_.compare_exchange_weak(e, end_ptr)) {
        ...
        if (is_dummy(e)) {
          ...
          BOOST_ACTOR_ASSERT(end_ptr == nullptr);
          return false;
        }
        while (e) {
          BOOST_ACTOR_ASSERT(! is_dummy(e));
          auto next = e->next;
          e->next = head_;
          head_ = e;
          e = next;
...
#endif // BOOST_ACTOR_DETAIL_SINGLE_READER_QUEUE_HPP
