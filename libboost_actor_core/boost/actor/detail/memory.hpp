...
#ifndef BOOST_ACTOR_DETAIL_MEMORY_HPP
#define BOOST_ACTOR_DETAIL_MEMORY_HPP
...
struct rc_storage : public ref_counted {
  ...
  rc_storage(Ts&&... xs)
      : instance(intrusive_ptr<ref_counted>(this, false),
        std::forward<Ts>(xs)...) {
    BOOST_ACTOR_ASSERT(get_reference_count() >= 1);
  }
};
...
public:
  ...
  embedded_storage new_embedded_storage() override {
    ...
    if (! cache_) {
      ...
      BOOST_ACTOR_ASSERT(cache_->unique());
    }
...
#endif // BOOST_ACTOR_DETAIL_MEMORY_HPP
