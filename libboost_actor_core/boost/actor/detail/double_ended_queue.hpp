...
#ifndef BOOST_ACTOR_DETAIL_DOUBLE_ENDED_QUEUE_HPP
#define BOOST_ACTOR_DETAIL_DOUBLE_ENDED_QUEUE_HPP
...
// GCC hack
#if defined(BOOST_ACTOR_GCC) && ! defined(_GLIBCXX_USE_SCHED_YIELD)
#include <time.h>
namespace std {
...
// another GCC hack
#if defined(BOOST_ACTOR_GCC) && ! defined(_GLIBCXX_USE_NANOSLEEP)
#include <time.h>
namespace std {
namespace this_thread {
namespace {
...
public:
  ...
  void append(pointer value) {
    BOOST_ACTOR_ASSERT(value != nullptr);
  ...
  pointer take_tail() {
    ...
    { // lifetime scope of guards
      lock_guard guard1(head_lock_);
      lock_guard guard2(tail_lock_);
      BOOST_ACTOR_ASSERT(head_ != nullptr);
      ...
      BOOST_ACTOR_ASSERT(tail_ != nullptr);
...
private:
  ...
  char pad1_[BOOST_ACTOR_CACHE_LINE_SIZE - sizeof(node*)];
  // guarded by tail_lock_
  std::atomic<node*> tail_;
  char pad2_[BOOST_ACTOR_CACHE_LINE_SIZE - sizeof(node*)];
...
#endif // BOOST_ACTOR_DETAIL_DOUBLE_ENDED_QUEUE_HPP
