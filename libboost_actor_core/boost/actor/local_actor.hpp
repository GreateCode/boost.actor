...
#ifndef BOOST_ACTOR_LOCAL_ACTOR_HPP
#define BOOST_ACTOR_LOCAL_ACTOR_HPP
...
#include "boost/actor/invoke_message_result.hpp"
...
public:
  ...
  template <class T, spawn_options Os = no_spawn_options, class... Ts>
  BOOST_ACTOR_DEPRECATED typename actor_handle_from_signature_list<
  ...
  spawn_typed(F fun, Ts&&... xs) {
    constexpr auto os = make_unbound(Os);
    auto res = boost::actor::spawn_typed_functor<os>(host(),
  ...
  message& last_dequeued() BOOST_ACTOR_DEPRECATED;

  actor_addr& last_sender() BOOST_ACTOR_DEPRECATED;
...
#endif // BOOST_ACTOR_LOCAL_ACTOR_HPP
