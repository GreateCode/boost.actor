...
#include "boost/actor/detail/try_match.hpp"
...
bool match_element(const meta_element& me, const message& msg,
  ...
  BOOST_ACTOR_ASSERT(me.typenr != 0 || me.type != nullptr);
...
bool match_atom_constant(const meta_element& me, const message& msg,
                         size_t pos, void** storage) {
  BOOST_ACTOR_ASSERT(me.typenr == detail::type_nr<atom_value>::value);
...
bool try_match(const message& msg, pattern_iterator pb, size_t ps, void** out) {
  BOOST_ACTOR_ASSERT(out != nullptr);
  BOOST_ACTOR_ASSERT(msg.empty() || msg.vals()->get_reference_count() > 0);
