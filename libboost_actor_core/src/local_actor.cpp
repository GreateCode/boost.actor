...
void local_actor::handle_timeout(behavior& bhvr, uint32_t timeout_id) {
  ...
  if (is_blocking()) {
    BOOST_ACTOR_ASSERT(bhvr_stack_.back() == bhvr);
  ...
  if (msg.size() > 1 && msg.match_element<sys_atom>(0) && node.sender) {
    ...
    msg.apply({
      ...
      [&](sys_atom, get_atom, std::string& what) {
        BOOST_ACTOR_LOGF_TRACE(BOOST_ACTOR_ARG(what));
        if (what == "info") {
          BOOST_ACTOR_LOGF_DEBUG("reply to 'info' message");
          node.sender->enqueue(
            mailbox_element::make_joint(self->address(), node.mid.response_id(),
...
bool handle_message_id_res(local_actor* self, message& res,
  ...
  if (res.match_elements<atom_value, uint64_t>()
    ...
    BOOST_ACTOR_LOGF_DEBUG("message handler returned a message id wrapper");
...
optional<message> post_process_invoke_res(local_actor* self,
                                          ...
                                          Handle hdl = Handle{}) {
  BOOST_ACTOR_LOGF_TRACE(BOOST_ACTOR_MARG(mid, integer_value) << ", " << BOOST_ACTOR_TSARG(res));
  if (! res) {
    return none;
  }
  if (res->empty()) {
  ...
  BOOST_ACTOR_LOGF_DEBUG("respond via response_promise");
  auto fhdl = fetch_response_promise(self, hdl);
  if (fhdl) {
...
invoke_message_result local_actor::invoke_message(mailbox_element_ptr& ptr,
  ...
  BOOST_ACTOR_ASSERT(ptr != nullptr);
  BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_TSARG(*ptr) << ", " << BOOST_ACTOR_MARG(awaited_id, integer_value));
  switch (filter_msg(this, *ptr)) {
    ...
    case msg_type::sys_message:
      BOOST_ACTOR_LOG_DEBUG("handled system message");
      return im_dropped;
    ...
    case msg_type::timeout: {
      ...
      BOOST_ACTOR_LOG_DEBUG("async timeout ignored while in sync mode");
      return im_dropped;
    }
    case msg_type::sync_response:
      BOOST_ACTOR_LOG_DEBUG("handle as synchronous response: "
                    << BOOST_ACTOR_TARG(ptr->msg, to_string) << ", "
                    << BOOST_ACTOR_MARG(ptr->mid, integer_value) << ", "
                    << BOOST_ACTOR_MARG(awaited_id, integer_value));
    ...
    case msg_type::ordinary:
      ...
      BOOST_ACTOR_LOG_DEBUG_IF(awaited_id.valid(),
...
void local_actor::mark_arrived(message_id mid) {
  BOOST_ACTOR_ASSERT(mid.is_response());
...
void local_actor::launch(execution_unit* eu, bool lazy, bool hide) {
  ...
  if (is_detached()) {
    ...
    BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_ARG(lazy) << ", " << BOOST_ACTOR_ARG(hide));
    ...
    std::thread{[hide](intrusive_ptr<local_actor> mself) {
      ...
      // destructed before dec_detached_threads() is called
      {
        ...
        while (mself->resume(nullptr, max_throughput) != resumable::done) {
          ...
          mself->await_data();
          BOOST_ACTOR_ASSERT(mself->mailbox().blocked() == false);
        }
        mself.reset();
      }
...
resumable::resume_result local_actor::resume(execution_unit* eu,
  ...
  if (is_blocking()) {
    // actor lives in its own thread
    BOOST_ACTOR_ASSERT(dynamic_cast<blocking_actor*>(this) != 0);
  ...
  auto actor_done = [&]() -> bool {
    if (! has_behavior() || planned_exit_reason() != exit_reason::not_exited) {
      BOOST_ACTOR_LOG_DEBUG("actor either has no behavior or has set an exit reason");
      on_exit();
      bhvr_stack().clear();
      bhvr_stack().cleanup();
  ...
  BOOST_ACTOR_ASSERT(! is_initialized()
             || (has_behavior()
                 && planned_exit_reason() == exit_reason::not_exited));
  std::exception_ptr eptr = nullptr;
  try {
    if (! is_initialized()) {
      BOOST_ACTOR_LOG_DEBUG("initialize actor");
      initialize();
      if (actor_done()) {
        BOOST_ACTOR_LOG_DEBUG("actor_done() returned true right "
                      << "after make_behavior()");
    ...
    for (size_t i = 0; i < max_throughput; ++i) {
      ...
      if (ptr) {
        ...
        BOOST_ACTOR_LOG_DEBUG("no more element in mailbox; going to block");
        reset_timeout_if_needed();
        if (mailbox().try_block()) {
          return resumable::awaiting_message;
        }
        BOOST_ACTOR_LOG_DEBUG("try_block() interrupted by new message");
      }
    }
    reset_timeout_if_needed();
    ...
    BOOST_ACTOR_LOG_INFO("actor died because of an exception, what: " << e.what());
    ...
    BOOST_ACTOR_LOG_INFO("actor died because of an unknown exception");
    if (exit_reason() == exit_reason::not_exited) {
      quit(exit_reason::unhandled_exception);
    }
    eptr = std::current_exception();
