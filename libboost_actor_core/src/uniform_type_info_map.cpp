...
void process(serializer& sink, const channel& chref) {
  ...
  if (! chref) {
    ...
    BOOST_ACTOR_ASSERT(rptr->is_abstract_actor() || rptr->is_abstract_group());
...
public:
  explicit default_meta_message(const std::string& tname) : name_(tname) {
    ...
    BOOST_ACTOR_ASSERT(elements.size() > 0 && elements.front() == "@<>");
  ...
  void serialize(const void* ptr, serializer* sink) const override {
    auto& msg = *cast(ptr);
    BOOST_ACTOR_ASSERT(msg.size() == elements_.size());
...
public:
  void initialize() {
    ...
    BOOST_ACTOR_ASSERT(std::is_sorted(builtin_types_.begin(),
                               builtin_types_.end(),
