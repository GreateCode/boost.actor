...
#if defined(BOOST_ACTOR_LINUX) || defined(BOOST_ACTOR_MACOS)
#include <unistd.h>
#include <cxxabi.h>
#include <sys/types.h>
#endif
...
#include "boost/actor/detail/get_process_id.hpp"
...
#else
  constexpr int global_log_level = BOOST_ACTOR_LOG_LEVEL;
#endif
...
public:
  ...
  void operator()() {
    ...
    for (;;) {
      ...
      BOOST_ACTOR_ASSERT(event != nullptr);
      if (event->msg.empty()) {
        out.close();
        return;
      }
