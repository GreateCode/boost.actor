...
public:
  ...
  std::pair<bool, size_t> erase_subscriber(const actor_addr& who) {
    BOOST_ACTOR_LOG_TRACE(""); // serializing who would cause a deadlock
    exclusive_guard guard(mtx_);
  ...
  behavior make_behavior() override {
    return {
      [=](join_atom, const actor& other) {
        BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_TSARG(other));
      ...
      [=](const down_msg&) {
        ...
        BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_TSARG(sender));
        if (sender) {
      ...
      others >> [=] {
        ...
        BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_TSARG(msg));
...
private:
  void send_to_acquaintances(const message& what) {
    ...
    auto sender = current_sender();
    BOOST_ACTOR_LOG_DEBUG("forward message to " << acquaintances_.size()
...
public:
  ...
  local_group_proxy(actor remote_broker, Ts&&... xs)
    ...
    BOOST_ACTOR_ASSERT(broker_ == invalid_actor);
    BOOST_ACTOR_ASSERT(remote_broker != invalid_actor);
  ...
  attachable_ptr subscribe(const actor_addr& who) override {
    ...
    BOOST_ACTOR_LOG_WARNING("actor already joined group");
    return {};
  }
...
public:
  ...
  void serialize(local_group* ptr, serializer* sink) {
    ...
    BOOST_ACTOR_ASSERT(ptr->broker() != invalid_actor);
