...
void abstract_actor::attach(attachable_ptr ptr) {
  ...
  BOOST_ACTOR_LOG_DEBUG("cannot attach functor to terminated actor: call immediately");
...
size_t abstract_actor::detach_impl(const attachable::token& what,
  ...
  if (! ptr) {
    BOOST_ACTOR_LOGF_DEBUG("invalid ptr");
    return 0;
  }
  if (ptr->matches(what)) {
    if (! dry_run) {
      BOOST_ACTOR_LOGF_DEBUG("removed element");
      attachable_ptr next;
      next.swap(ptr->next);
      ptr.swap(next);
    }
...
bool abstract_actor::link_impl(linking_operation op, const actor_addr& other) {
  BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_ARG(op) << ", " << BOOST_ACTOR_TSARG(other));
  switch (op) {
    ...
    default:
      BOOST_ACTOR_ASSERT(op == remove_backlink_op);
...
void abstract_actor::cleanup(uint32_t reason) {
  ...
  BOOST_ACTOR_ASSERT(reason != exit_reason::not_exited);
  ...
  BOOST_ACTOR_LOG_INFO_IF(! is_remote(), "cleanup actor with ID " << id_
                                << "; exit reason = " << reason);
  // send exit messages
