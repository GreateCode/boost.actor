...
#define BOOST_TEST_MODULE sync_send
    ...
    return {
      [=](go_atom, const actor& next) {
        ...
        sync_send(next, gogo_atom::value).then(
          [=](atom_value) {
            BOOST_TEST_MESSAGE("send `ok_atom` to buddy");
...
BOOST_AUTO_TEST_CASE(test_void_res) {
  ...
  self->sync_send(buddy, 1, 2).await([] {
    BOOST_TEST_MESSAGE("received void res");
  });
  self->send_exit(buddy, exit_reason::kill);
}

BOOST_AUTO_TEST_CASE(pending_quit) {
  ...
  spawn([mirror](event_based_actor* self) {
    self->sync_send(mirror, 42).then(
      others >> [] {
        BOOST_ERROR("received result, should've been terminated already");
      }
    );
    self->quit();
  });
...
BOOST_AUTO_TEST_CASE(sync_send) {
  scoped_actor self;
  self->on_sync_failure([&] {
    BOOST_ERROR("received: " << to_string(self->current_message()));
  });
  self->spawn<monitored + blocking_api>([](blocking_actor* s) {
    ...
    s->on_sync_failure([=] {
      BOOST_ERROR("received: " << to_string(s->current_message()));
    });
    ...
    BOOST_CHECK_EQUAL(sync_failure_called, true);
    BOOST_CHECK_EQUAL(int_handler_called, false);
  ...
  BOOST_CHECK_EQUAL(continuation_called, true);
  self->send_exit(mirror, exit_reason::user_shutdown);
  BOOST_TEST_MESSAGE("block on `await_all_other_actors_done");
  self->await_all_other_actors_done();
  BOOST_TEST_MESSAGE("`await_all_other_actors_done` finished");
  auto await_ok_message = [&] {
    self->receive(
      [](ok_atom) {
        BOOST_TEST_MESSAGE("received `ok_atom`");
      },
      [](error_atom) {
        BOOST_ERROR("A didn't receive sync response");
      },
      [&](const down_msg& dm) -> optional<skip_message_t> {
        if (dm.reason == exit_reason::normal) {
          return skip_message();
        }
        BOOST_ERROR("A exited for reason " << dm.reason);
        return none;
      }
    );
  };
  ...
  BOOST_TEST_MESSAGE("block on `await_ok_message`");
  await_ok_message();
  BOOST_TEST_MESSAGE("`await_ok_message` finished");
  self->await_all_other_actors_done();
  self->send(self->spawn<A, monitored>(self),
  ...
  self->sync_send(self, no_way_atom::value).await(
    ...
    after(milliseconds(50)) >> [] {
      BOOST_TEST_MESSAGE("Got timeout");
    }
  );
  ...
  self->receive_for(i, 3)(
    ...
    [](no_way_atom) {
      BOOST_TEST_MESSAGE("trigger \"actor did not reply to a "
  ...
  self->receive(
    ...
    after(milliseconds(0)) >> [] {
      BOOST_TEST_MESSAGE("Mailbox is empty, all good");
    }
  );
  ...
  self->sync_send(c, milliseconds(500), hi_there_atom::value).await(
    [&](hi_there_atom) {
      BOOST_ERROR("C did reply to 'HiThere'");
    },
    after(milliseconds(500)) >> [&] {
      BOOST_TEST_MESSAGE("timeout occured");
      timeout_occured = true;
    }
  );
  BOOST_CHECK_EQUAL(timeout_occured, true);
  self->on_sync_failure([&] {
    BOOST_ERROR("Unexpected message: "
                   << to_string(self->current_message()));
  });
  self->sync_send(c, gogo_atom::value).await(
    [](gogogo_atom) {
      BOOST_TEST_MESSAGE("received `gogogo_atom`");
    }
  );
  ...
  self->spawn<monitored + blocking_api>([](blocking_actor* s) { // client
    ...
    s->sync_send(serv, request_atom::value).await(
      [=](response_atom) {
        BOOST_TEST_MESSAGE("received `response_atom`");
        BOOST_CHECK(s->current_sender() == work);
      },
      others >> [&] {
        BOOST_ERROR("Unexpected message: "
                       << to_string(s->current_message()));
...
BOOST_AUTO_TEST_CASE(sync_send_no_then) {
