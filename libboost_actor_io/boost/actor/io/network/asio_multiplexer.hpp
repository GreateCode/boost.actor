...
#ifndef BOOST_ACTOR_IO_NETWORK_ASIO_MULTIPLEXER_HPP
#define BOOST_ACTOR_IO_NETWORK_ASIO_MULTIPLEXER_HPP
...
public:
  ...
  void start(const manager_ptr& mgr) {
    BOOST_ACTOR_ASSERT(mgr != nullptr);
    read_loop(mgr);
  }
...
private:
  void read_loop(const manager_ptr& mgr) {
    auto cb = [=](const boost::system::error_code& ec, size_t read_bytes) {
      BOOST_ACTOR_LOGC(BOOST_ACTOR_TRACE, "boost::actor::io::network::stream", "read_loop$cb",
  ...
  void write_loop(const manager_ptr& mgr) {
    ...
    boost::asio::async_write(
      ...
      [=](const boost::system::error_code& ec, size_t nb) {
        BOOST_ACTOR_LOGC(BOOST_ACTOR_TRACE, "boost::actor::io::network::stream", "write_loop$lambda",
                 BOOST_ACTOR_ARG(this));
        static_cast<void>(nb); // silence compiler warning
        if (! ec) {
          BOOST_ACTOR_LOGC(BOOST_ACTOR_DEBUG, "boost::actor::io::network::stream", "write_loop$lambda",
                   nb << " bytes sent");
          write_loop(mgr);
        } else {
          BOOST_ACTOR_LOGC(BOOST_ACTOR_DEBUG, "boost::actor::io::network::stream", "write_loop$lambda",
  ...
  void collect_data(const manager_ptr& mgr, size_t collected_bytes) {
    fd_.async_read_some(boost::asio::buffer(rd_buf_.data() + collected_bytes,
                         ...
                         [=](const boost::system::error_code& ec, size_t nb) {
      BOOST_ACTOR_LOGC(BOOST_ACTOR_TRACE, "boost::actor::io::network::stream", "collect_data$lambda",
               BOOST_ACTOR_ARG(this));
      if (! ec) {
...
#endif // BOOST_ACTOR_IO_NETWORK_ASIO_MULTIPLEXER_HPP
