...
basp_broker_state::basp_broker_state(broker* selfptr)
  ...
  BOOST_ACTOR_ASSERT(this_node() != invalid_node_id);
}
...
actor_proxy_ptr basp_broker_state::make_proxy(const node_id& nid,
                                              actor_id aid) {
  BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_TSARG(nid) << ", " << BOOST_ACTOR_ARG(aid));
  BOOST_ACTOR_ASSERT(nid != this_node());
  ...
  if (! path) {
    ...
    BOOST_ACTOR_LOG_INFO("cannot create a proxy instance for an actor "
...
void basp_broker_state::finalize_handshake(const node_id& nid, actor_id aid,
  ...
  BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_TSARG(nid)
                << ", " << BOOST_ACTOR_ARG(aid)
                << ", " << BOOST_ACTOR_TSARG(make_message(sigs)));
  BOOST_ACTOR_ASSERT(this_context != nullptr);
  this_context->id = nid;
  ...
  if (nid == this_node()) {
    ...
    BOOST_ACTOR_LOG_INFO_IF(! ptr, "actor with ID " << aid << " not found in registry");
  } else {
    ptr = namespace_.get_or_put(nid, aid);
    //ptr = make_proxy(nid, aid);
    BOOST_ACTOR_LOG_ERROR_IF(! ptr, "creating actor in finalize_handshake failed");
  }
...
void basp_broker_state::proxy_announced(const node_id& nid, actor_id aid) {
  ...
  auto send_kill_proxy_instance = [=](uint32_t rsn) {
    auto path = instance.tbl().lookup(nid);
    if (! path) {
      BOOST_ACTOR_LOG_INFO("cannot send exit message for proxy, no route to host "
                   << to_string(nid));
      return;
    }
  ...
  if (entry.second != exit_reason::not_exited) {
    BOOST_ACTOR_LOG_DEBUG("kill proxy immediately");
...
void basp_broker_state::kill_proxy(const node_id& nid, actor_id aid,
                                   uint32_t rsn) {
  BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_TSARG(nid) << ", " << BOOST_ACTOR_ARG(aid) << ", " << BOOST_ACTOR_ARG(rsn));
...
void basp_broker_state::deliver(const node_id& source_node,
                                ...
                                message& msg,
                                message_id mid) {
  BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_TSARG(source_node) << ", " << BOOST_ACTOR_ARG(source_actor)
                << ", " << BOOST_ACTOR_TSARG(dest_node)<< ", " << BOOST_ACTOR_ARG(dest_actor)
  ...
  if (dest_node != this_node())
    ...
    if (dest_actor == std::numeric_limits<actor_id>::max()) {
      ...
      BOOST_ACTOR_LOG_DEBUG("dest_name = " << to_string(dest_name));
  ...
  if (! dest) {
    BOOST_ACTOR_LOG_INFO("cannot deliver message, destination not found");
...
void basp_broker_state::learned_new_node(const node_id& nid) {
    ...
    return {
      [=](ok_atom, const std::string& /* key == "info" */,
        ...
        this_actor->become(
          ...
          others >> [=] {
            BOOST_ACTOR_LOGF_ERROR("spawn server has received unexpected message: "
                           << to_string(this_actor->current_message()));
          }
        );
      },
      after(std::chrono::minutes(5)) >> [=] {
        BOOST_ACTOR_LOGF_INFO("no spawn server on node " << to_string(nid));
        this_actor->quit();
      }
    };
  });
  ...
  auto path = instance.tbl().lookup(nid);
  if (! path) {
    BOOST_ACTOR_LOG_ERROR("learned_new_node called, but no route to nid");
    return;
  }
...
void basp_broker_state::learned_new_node_indirectly(const node_id& nid) {
  ...
  auto connection_helper = [=](event_based_actor* helper, actor s) -> behavior {
    ...
    return {
      ...
      [=](ok_atom, const std::string&, message& msg) {
        BOOST_ACTOR_LOGF_DEBUG("received requested config: " << to_string(msg));
        ...
        msg.apply({
          [&](uint16_t port, network::address_listing& addresses) {
              ...
              if (kvp.first != network::protocol::ethernet)
                for (auto& addr : kvp.second) {
                  try {
                    ...
                    BOOST_ACTOR_LOGF_INFO("connected directly via " << addr);
            ...
            BOOST_ACTOR_LOGF_INFO("could not connect to node directly, nid = "
                          << to_string(nid));
          }
        });
      },
      ...
      after(std::chrono::minutes(10)) >> [=] {
        ...
        BOOST_ACTOR_LOGF_INFO("aborted direct connection attempt after 10min, nid = "
                      << to_string(nid));
  ...
  auto path = instance.tbl().lookup(nid);
  if (! path) {
    BOOST_ACTOR_LOG_ERROR("learned_new_node_indirectly called, but no route to nid");
    return;
  }
  if (path->next_hop == nid) {
    BOOST_ACTOR_LOG_ERROR("learned_new_node_indirectly called with direct connection");
    return;
  }
  using namespace detail;
...
void basp_broker_state::set_context(connection_handle hdl) {
  ...
  auto i = ctx.find(hdl);
  if (i == ctx.end()) {
    BOOST_ACTOR_LOG_INFO("create new BASP context for handle " << hdl.id());
    i = ctx.emplace(hdl,
                    connection_context{
                      basp::await_header,
...
bool basp_broker_state::erase_context(connection_handle hdl) {
  ...
  auto& ref = i->second;
  if (ref.callback) {
    BOOST_ACTOR_LOG_DEBUG("connection closed during handshake");
...
behavior basp_broker::make_behavior() {
  ...
  return {
    ...
    [=](new_data_msg& msg) {
      BOOST_ACTOR_LOG_TRACE("handle = " << msg.handle.id());
    ...
    [=](forward_atom, const actor_addr& sender, const actor_addr& receiver,
      ...
      BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_TSARG(sender) << ", " << BOOST_ACTOR_TSARG(receiver)
                    << ", " << BOOST_ACTOR_MARG(mid, integer_value)
                    << ", " << BOOST_ACTOR_TSARG(msg));
      if (receiver == invalid_actor_addr || ! receiver.is_remote()) {
        BOOST_ACTOR_LOG_WARNING("cannot forward to invalid or local actor: "
                        << to_string(receiver));
        return;
      }
    ...
    [=](const connection_closed_msg& msg) {
      BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_MARG(msg.handle, id));
      ...
      BOOST_ACTOR_ASSERT(nid == invalid_node_id
    ...
    [=](publish_atom, accept_handle hdl, uint16_t port, const actor_addr& whom,
        std::set<std::string>& sigs) {
      BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_ARG(hdl.id()) << ", "<< BOOST_ACTOR_TSARG(whom)
      ...
      try {
        ...
        BOOST_ACTOR_LOG_DEBUG("failed to assign doorman from handle");
        return;
      }
    ...
    [=](connect_atom, connection_handle hdl, uint16_t port) {
      BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_ARG(hdl.id()));
      auto rp = make_response_promise();
      try {
        assign_tcp_scribe(hdl);
      }
      catch (std::exception& e) {
        BOOST_ACTOR_LOG_DEBUG("failed to assign scribe from handle: " << e.what());
    ...
    [=](ok_atom, const std::string& key, message& value) {
      if (key == "global.enable-automatic-connections") {
        value.apply([&](bool enabled) {
          if (! enabled)
            return;
          BOOST_ACTOR_LOG_INFO("enable automatic connection");
    ...
    others >>
    [=] {
      BOOST_ACTOR_LOGF_ERROR("received unexpected message: "
