...
void routing_table::add_direct(const connection_handle& hdl,
                               const node_id& nid) {
  BOOST_ACTOR_ASSERT(direct_by_hdl_.count(hdl) == 0);
  BOOST_ACTOR_ASSERT(direct_by_nid_.count(nid) == 0);
...
instance::instance(abstract_broker* parent, callee& lstnr)
    : tbl_(parent),
      this_node_(boost::actor::detail::singletons::get_node_id()),
      callee_(lstnr) {
  BOOST_ACTOR_ASSERT(this_node_ != invalid_node_id);
}
...
connection_state instance::handle(const new_data_msg& dm, header& hdr,
  ...
  if (is_payload) {
    ...
    if (payload->size() != hdr.payload_len) {
      BOOST_ACTOR_LOG_WARNING("received invalid payload");
      return err();
    }
  } else {
  ...
  BOOST_ACTOR_LOG_DEBUG("hdr = " << to_string(hdr));
  if (! valid(hdr)) {
    BOOST_ACTOR_LOG_WARNING("received invalid header of type "
                    << to_string(hdr.operation));
    return err();
  }
  // needs forwarding?
  if (! is_handshake(hdr) && hdr.dest_node != this_node_) {
    ...
    if (path) {
      ...
      BOOST_ACTOR_LOG_INFO("cannot forward message, no route to destination");
      if (hdr.source_node != this_node_) {
        auto reverse_path = lookup(hdr.source_node);
        if (! reverse_path) {
          BOOST_ACTOR_LOG_WARNING("cannot send error message: no route to source");
        } else {
        ...
        BOOST_ACTOR_LOG_WARNING("lost packet with probably spoofed source");
      }
  ...
  switch (hdr.operation) {
    case message_type::server_handshake: {
      ...
      if (hdr.source_node == this_node_) {
        BOOST_ACTOR_LOG_INFO("close connection to self immediately");
      ...
      if (tbl_.lookup_direct(hdr.source_node) != invalid_connection_handle) {
        BOOST_ACTOR_LOG_INFO("close connection since we already have a "
                     "direct connection to " << to_string(hdr.source_node));
        callee_.finalize_handshake(hdr.source_node, aid, sigs);
        return err();
      }
      // add direct route to this node and remove any indirect entry
      BOOST_ACTOR_LOG_INFO("new direct connection: " << to_string(hdr.source_node));
      ...
      if (!path) {
        BOOST_ACTOR_LOG_ERROR("no route to host after server handshake");
        return err();
      }
    ...
    case message_type::client_handshake: {
      if (tbl_.lookup_direct(hdr.source_node) != invalid_connection_handle) {
        BOOST_ACTOR_LOG_INFO("received second client handshake for "
    ...
    case message_type::kill_proxy_instance:
      ...
      break;
    default:
      BOOST_ACTOR_LOG_ERROR("invalid operation");
      return err();
  }
  return await_header;
}
...
void instance::handle_node_shutdown(const node_id& affected_node) {
  BOOST_ACTOR_LOG_TRACE(BOOST_ACTOR_TSARG(affected_node));
  if (affected_node == invalid_node_id)
    return;
  BOOST_ACTOR_LOG_INFO("lost direct connection to " << to_string(affected_node));
  auto cb = make_callback([&](const node_id& nid){
    callee_.purge_state(nid);
  });
...
void instance::write(const routing_table::route& r, header& hdr,
  ...
  BOOST_ACTOR_ASSERT(hdr.payload_len == 0 || writer != nullptr);
...
bool instance::dispatch(const actor_addr& sender, const actor_addr& receiver,
  ...
  BOOST_ACTOR_LOG_TRACE("");
  BOOST_ACTOR_ASSERT(receiver.is_remote());
