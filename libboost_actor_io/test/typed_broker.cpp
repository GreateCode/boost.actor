...
#define BOOST_TEST_MODULE io_typed_broker
...
#include "boost/actor/io/experimental/typed_broker.hpp"
...
using namespace boost::actor::io::experimental;

namespace {

using publish_atom = atom_constant<atom("publish")>;
using ping_atom = boost::actor::atom_constant<atom("ping")>;
using pong_atom = boost::actor::atom_constant<atom("pong")>;
using kickoff_atom = boost::actor::atom_constant<atom("kickoff")>;
...
BOOST_AUTO_TEST_CASE(test_typed_broker) {
...
#   endif // BOOST_ACTOR_USE_ASIO
  ...
  if (r.opts.count("client-port") > 0) {
    ...
    BOOST_TEST_MESSAGE("spawn_io_client_typed...");
    auto cl = spawn_io_client(peer_fun, "localhost", port, p);
    BOOST_TEST_MESSAGE("spawn_io_client_typed finished");
